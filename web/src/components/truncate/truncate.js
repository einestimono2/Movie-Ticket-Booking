...
/**
 * @param {int} lines Chỉ định số dòng văn bản sẽ được giữ nguyên cho đến khi nó bị cắt bớt (default: 1)
 * @param {string | import('react').ReactNode} ellipsis Phần tử cuối văn bản trong trường hợp nó bị cắt bớt (default: ...)
 * @param {string | import('react').ReactNode} children Văn bản hoặc phần tử cần cắt ngắn
 * @param {boolean} trimWhitespace Xóa khoảng trắng trước ellipsis hay không? (default: false)
 * @param {boolean} addBr Thêm thẻ <br/> khi cắt hay không?
 */
export class Truncate extends Component {
    ...
    for (let line = 1; line <= numLines; line++) {
      ...
      if (line === numLines) {
        ...
        while (lower <= upper) {
          ...
          if (measureWidth(testLine) + ellipsisWidth + measureWidth(ellipsis?.props?.children ?? '') <= targetWidth) {
            lower = middle + 1;
          } else {
            upper = middle - 1;
          }
        ...
        // let lastLineText = textRest.slice(0, lower - ellipsis?.props?.children?.length ?? 0);
  ...
  renderLine(line, i, arr) {
    if (i === arr.length - 1) {
      ...
      if (line) {
        return this.props.addBr ? [<span key={i}>{line}</span>, br] : [<span key={i}>{`${line} `}</span>];
      } else {
        return br;
      }
    }
